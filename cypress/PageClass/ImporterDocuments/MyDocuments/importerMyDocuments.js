class importerMyDocuments
{
    ...
    enterFacilityProductDetails(facility,product)
    {
        // cy.contains("Select Supplier").click({force:true})
        // cy.wait(1000)
        cy.get(this.selectfacilityDD).select(facility,{force:true}).then(()=>{
            cy.get(this.selectfacilityDD+" option:selected").invoke('text').should('eq',facility)
        })
        cy.wait(2000)
        cy.get(this.selectProductDD).click().then(()=>{
            cy.get(this.productDDListCheckbox).check(product).should('be.checked')
            cy.get(this.newDocForm).click()
        })
